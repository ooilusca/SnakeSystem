inherited frmFormFiltroFornecedores: TfrmFormFiltroFornecedores
  Caption = 'FILTRO DE FORNECEDORES'
  ClientHeight = 427
  ClientWidth = 925
  FormStyle = fsMDIChild
  Visible = True
  WindowState = wsMaximized
  ExplicitWidth = 941
  ExplicitHeight = 466
  PixelsPerInch = 96
  TextHeight = 13
  inherited Panel1: TPanel
    Width = 925
    Height = 57
    BevelOuter = bvNone
    Color = 15452323
    ParentBackground = False
    ExplicitWidth = 925
    ExplicitHeight = 57
    inherited Label1: TLabel
      Left = 13
      Top = 12
      Caption = 'Nome:'
      ExplicitLeft = 13
      ExplicitTop = 12
    end
    object Label2: TLabel [1]
      Left = 244
      Top = 12
      Width = 29
      Height = 13
      Caption = 'CNPJ:'
    end
    inherited cxEdtFiltro: TcxTextEdit
      Left = 13
      Top = 27
      ExplicitLeft = 13
      ExplicitTop = 27
      ExplicitWidth = 225
      Width = 225
    end
    inherited Panel2: TPanel
      Left = 812
      Top = 0
      Height = 57
      ExplicitLeft = 812
      ExplicitTop = 0
      ExplicitHeight = 57
      inherited cxButton1: TcxButton
        Height = 47
        Font.Height = -12
        Font.Style = [fsBold]
        ParentFont = False
        ExplicitHeight = 47
      end
    end
    inherited btnFiltro: TcxButton
      AlignWithMargins = True
      Left = 442
      Top = 5
      Width = 100
      Height = 47
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 5
      Margins.Bottom = 5
      Align = alRight
      Font.Height = -12
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = btnFiltroClick
      ExplicitLeft = 442
      ExplicitTop = 5
      ExplicitWidth = 100
      ExplicitHeight = 47
    end
    object cxEdtCNPJ: TcxTextEdit
      Left = 244
      Top = 27
      Style.LookAndFeel.NativeStyle = False
      Style.LookAndFeel.SkinName = 'Office2010Blue'
      StyleDisabled.LookAndFeel.NativeStyle = False
      StyleDisabled.LookAndFeel.SkinName = 'Office2010Blue'
      StyleFocused.LookAndFeel.NativeStyle = False
      StyleFocused.LookAndFeel.SkinName = 'Office2010Blue'
      StyleHot.LookAndFeel.NativeStyle = False
      StyleHot.LookAndFeel.SkinName = 'Office2010Blue'
      TabOrder = 3
      Width = 134
    end
    object cxbtnNovo: TcxButton
      AlignWithMargins = True
      Left = 552
      Top = 5
      Width = 100
      Height = 47
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 5
      Margins.Bottom = 5
      Align = alRight
      Caption = 'Novo'
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'McSkin'
      OptionsImage.Glyph.SourceDPI = 96
      OptionsImage.Glyph.Data = {
        
        723D2231222F3E0D0A093C2F673E0D0A3C2F7376673E0D0A}
      TabOrder = 4
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = cxbtnNovoClick
    end
    object cxbtnVisualizar: TcxButton
      AlignWithMargins = True
      Left = 662
      Top = 5
      Width = 100
      Height = 47
      Margins.Left = 5
      Margins.Top = 5
      Margins.Right = 50
      Margins.Bottom = 5
      Align = alRight
      Caption = 'Visualizar'
      LookAndFeel.NativeStyle = False
      LookAndFeel.SkinName = 'McSkin'
      OptionsImage.Glyph.SourceDPI = 96
      OptionsImage.Glyph.Data = {
        
        C60000000049454E44AE426082}
      TabOrder = 5
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = cxbtnVisualizarClick
    end
  end
  object cxGridFornecedores: TcxGrid [1]
    Left = 0
    Top = 57
    Width = 925
    Height = 370
    Align = alClient
    BevelInner = bvNone
    BevelOuter = bvNone
    TabOrder = 1
    LookAndFeel.Kind = lfUltraFlat
    LookAndFeel.NativeStyle = False
    LookAndFeel.SkinName = 'MoneyTwins'
    object cxGridFornecedoresDBTableView1: TcxGridDBTableView
      Navigator.Buttons.CustomButtons = <>
      Navigator.InfoPanel.DisplayMask = '[RecordIndex] de [RecordCount]'
      FindPanel.InfoText = 'Digite algo para pesquisar...'
      DataController.DataSource = dsFiltro
      DataController.Summary.DefaultGroupSummaryItems = <>
      DataController.Summary.FooterSummaryItems = <
        item
          Kind = skCount
          FieldName = 'ID_FORNECEDOR'
          Column = cxGridFornecedoresDBTableView1Column2
          DisplayText = 'Total:'
        end>
      DataController.Summary.SummaryGroups = <>
      FilterRow.InfoText = 'Clique aqui para definir um filtro'
      OptionsView.NoDataToDisplayInfoText = '<Nenhum resultado de pesquisa>'
      OptionsView.Footer = True
      OptionsView.RowSeparatorWidth = 1
      object cxGridFornecedoresDBTableView1Column1: TcxGridDBColumn
        Caption = 'C'#243'd.'
        DataBinding.FieldName = 'ID_FORNECEDOR'
        Options.Editing = False
        Width = 40
      end
      object cxGridFornecedoresDBTableView1Column2: TcxGridDBColumn
        Caption = 'Fornecedor'
        DataBinding.FieldName = 'FORNECEDOR'
        Options.Editing = False
        Width = 200
      end
      object cxGridFornecedoresDBTableView1Column3: TcxGridDBColumn
        DataBinding.FieldName = 'CNPJ'
        Options.Editing = False
        Width = 100
      end
      object cxGridFornecedoresDBTableView1Column4: TcxGridDBColumn
        Caption = 'Contato'
        DataBinding.FieldName = 'CONTATO'
        Options.Editing = False
        Width = 250
      end
    end
    object cxGridFornecedoresLevel1: TcxGridLevel
      GridView = cxGridFornecedoresDBTableView1
    end
  end
  inherited FDFiltro: TFDQuery
    SQL.Strings = (
      'SELECT FNC.ID ID_FORNECEDOR, FNC.ID_CIDADE, '
      '       FNC.NOME FORNECEDOR, FNC.CNPJ, '
      '       FNC.CONTATO'
      '   FROM FORNECEDOR FNC'
      'WHERE FNC.DATA_EXCLUIDO IS NULL')
    Left = 128
    Top = 152
    object FDFiltroID_FORNECEDOR: TIntegerField
      Alignment = taCenter
      FieldName = 'ID_FORNECEDOR'
      Origin = 'ID'
      Required = True
    end
    object FDFiltroID_CIDADE: TIntegerField
      FieldName = 'ID_CIDADE'
      Origin = 'ID_CIDADE'
      Required = True
    end
    object FDFiltroFORNECEDOR: TStringField
      FieldName = 'FORNECEDOR'
      Origin = 'NOME'
      Required = True
      Size = 100
    end
    object FDFiltroCNPJ: TStringField
      FieldName = 'CNPJ'
      Origin = 'CNPJ'
      Size = 45
    end
    object FDFiltroCONTATO: TStringField
      FieldName = 'CONTATO'
      Origin = 'CONTATO'
      Size = 200
    end
  end
  inherited dsFiltro: TDataSource
    Left = 160
    Top = 152
  end
  inherited FDTFiltro: TFDTransaction
    Left = 192
    Top = 152
  end
end
